...
为降低上下文切换成本，项目在实现完成后还需要保留以下交接资产：

1. handoff.md
   记录当前阶段状态、关键设计决策、已验证结果目录和明确下一步。
2. 结果归因报告
   从 summary、scheduler_debug、schedule_history、link_load_trace 和 flow_trace 生成聚合报告，至少覆盖链路曲线摘要、阶段耗时摘要和 epoch 动作摘要。
3. 公平矩阵配置
   将规模扩展、负载敏感性和参数敏感性实验固化到单一机器可读文件，供后续上下文直接枚举。
4. prompt.md
   明确新上下文的读取顺序、Python 环境约束和会话结束动作。

这些资产的目标不是替代原始结果文件，而是提供“先读什么、现状如何、下一步做什么”的最短路径。
